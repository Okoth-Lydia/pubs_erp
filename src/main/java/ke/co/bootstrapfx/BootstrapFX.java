...
public class BootstrapFX  {
     ...
     *
     * TODO: NullPointerExceptions when getting text for null fields
     */
    @SafeVarargs
    protected static <Field extends Control> String[] getText(Field... f) {
        ...
        for (int i= 0; i<f.length; i++)
            if (f[i] instanceof TextField) {
                if (f[i].getStyleClass().contains("email")) {
                    if (isValid(f[i])) text[i]= ((TextField)f[i]).getText();
                } else text[i]= ((TextField)f[i]).getText();
